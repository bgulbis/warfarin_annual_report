...
Patients managed by the Pharmacy Dosing Service have a greater rise in INR over the first 7 days of therapy, and are more likely to reach an INR around 2 by day 5. 
...
There was a significantly higher number of patients who returned to a MH facility within 30 days in the traditional group, however, the percent of readmissions was not significantly different (see Table 1). There was no difference in acute hemoglobin decrease. 
...
tbl_hgb <- data_hgb_drop %>%
    ...
    filter(warfarin_start >= mdy("7/1/2016", tz = "US/Central"),
           warfarin_start <= mdy("6/30/2017", tz = "US/Central")) %>%
    group_by(group) %>%
    summarize_at("hgb_drop", mean, na.rm = TRUE) %>%
    mutate_at("hgb_drop", funs(. * 100)) %>%
    spread(group, hgb_drop) %>%
    mutate(Event = "Hemoglobin decrease by >/= 2 g/dL within a 48-hour period") 
...
prbc <- data_warfarin %>%
    ...
    left_join(data_prbc, by = "millennium.id") %>%
    filter(warfarin_start >= mdy("7/1/2016", tz = "US/Central"),
           warfarin_start <= mdy("6/30/2017", tz = "US/Central")) 
...
ffp <- data_warfarin %>%
    select(millennium.id, group, warfarin_start) %>%
    left_join(data_ffp, by = "millennium.id") %>%
    filter(warfarin_start >= mdy("7/1/2016", tz = "US/Central"),
           warfarin_start <= mdy("6/30/2017", tz = "US/Central")) 
...
```{r}
reversal <- data_warfarin %>%
    select(millennium.id, group, warfarin_start) %>%
    left_join(data_reversal, by = "millennium.id") %>%
    filter(warfarin_start >= mdy("7/1/2016", tz = "US/Central"),
           warfarin_start <= mdy("6/30/2017", tz = "US/Central")) 
...
```{r}
procedures <- data_warfarin %>%
    select(millennium.id, group, warfarin_start) %>%
    left_join(data_procedures, by = "millennium.id") %>%
    filter(warfarin_start >= mdy("7/1/2016", tz = "US/Central"),
           warfarin_start <= mdy("6/30/2017", tz = "US/Central")) 
...
tbl_revisit <- revisit_all %>%
    ...
    mutate(Event = "Revisit to any MH hospital within 30 days") 
...
tbl_readmit <- revisit_all %>%
    ...
    mutate(Event = "Readmission to any MH hospital within 30 days") 
...
tbl_hgb %>%
    ...
    mutate(`p-value` = p) %>%
    knitr::kable(digits = c(0, 1, 1, 2), caption = "Events during the first 7 days of warfarin therapy")
```
