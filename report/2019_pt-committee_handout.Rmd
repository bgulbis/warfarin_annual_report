---
title: "Review of warfarin dosing by the Pharmacy Dosing Service in fiscal year 2019"
...
There was a `r round(pct_warf_use$pct_chg, 0)`% decrease in the total number of warfarin doses administered in FY19 compared with FY17, due primarily to an increase in the use of Direct Oral Anticoagulants (DOACs), such as apixaban and rivaroxaban (see Figure 1). 
...
The highest volume of warfarin use occured in patients on the Cardiology service line, followed by Internal Medicine, CV Surgery, and Neurology (see Figure 2). The Cardiology service line continues to manage the majority of their own warfarin doses, although Clinical Pharmacists and Clinical Pharmacist-Specialists participate in the dosing of warfarin on these patients outside of a formal consult to the Pharmacy Dosing Service.

```{r, fig.cap="The number of orders for warfarin by the primary service at the time of warfarin initiation. The Cardiology service line remains the largest user of warfarin. The Pharmacy Dosing Service manages the majority of warfarin dosing for patients on all other service lines."}
df_warf_ord %>%
...
Patients who are on warfarin due to a ventricular assist device (VAD) are primarily managed through traditional provider dosing. The Pharmacy Dosing Service manages the majority of patients for all other indications (see Figure 3). 

```{r, fig.cap="The indications for oral anticoagulation. The most common indication for warfarin was atrial fibrillation (A.fib), followed by ventricular assist devices (VADs), mechanical heart valves, deep vein thrombosis (DVTs), and pulmonary embolisms (PEs)."}
df_demog %>%
    filter(fiscal_year == 2019) %>%
...
```{r, fig.cap="The daily warfarin dose administered to patients in each group. Patients in the Pharmacy Dosing Service group received a higher daily warfarin dose than those in the traditional provider management group. The p-value is < 0.01 for each day of warfarin therapy. Only patients with an INR target of 2 to 3 were included."}
p <- df_doses_inr %>%
...
Patients managed by the Pharmacy Dosing Service had a greater rise in INR over the first 10 days of therapy, and were more likely to reach an INR around 2 on day 5 (see Figure 5). 

```{r, fig.cap="The daily INR value for patients in each group over the first 10 days of warfarin therapy. There was a larger increase in the INR for patients managed by the Pharmacy Dosing Service. The p-value was < 0.05 on all days except days 5 and 6 where there was no significant difference. Only patients with an INR target of 2 to 3 were included."}
...
There was a larger percent of patients managed by the Pharmacy Dosing Service who were transferred to another facility for long-term care (see Table 1). 
...
df_outcomes %>%
    ...
    knitr::kable(
        ...
        caption = "Disposition of patient on discharge from MH-TMC. More patients in the pharmacy group required additional treatment at a LTAC, SNF, or Rehab facility."
    )
...
There were no differences in bleeding, need for anticoagulation reversal, readmission within 30 dyas, or other adverse events between groups (see Table 2).
...
df_outcomes_pct %>%
    knitr::kable(
        ...
        caption = paste(
            "Outcomes and events during the first 10 days of warfarin therapy.",
            ...
            "Unplanned return includes any ER visit, observation, or inpatient stay at any MH hospital within 30 days of discharge among patients who were discharged home.",
            sep = " "
        )
    ) 
```
