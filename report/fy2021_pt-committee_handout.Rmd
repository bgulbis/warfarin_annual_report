---
title: "Warfarin Pharmacy Dosing Service analysis for Fiscal Year 2021"
...
pct_warf_use <- df_cnt |>
    ...
    mutate(pct_chg = (`2019` - `2021`) / `2019` * 100)
```


# Summary

The Department of Pharmacy's Warfarin Dosing Service was consulted on `r round(n_consult_cur_fy$pct, 0)`% of warfarin patients in fiscal year `r cur_fy` (FY21). The service lines with the highest utilization were Cardiology, Internal Medicine, CV Surgery, and Neurology. Patients managed by the pharmacy service reached the target therapeutic range sooner than patients with traditional provider management of warfarin. 

# Warfarin Utilization

There was a `r round(pct_warf_use$pct_chg, 0)`% decrease in the total number of warfarin doses administered in FY21 compared with FY19. This was due to a combination of reduced volume due to COVID-19, as well as a continued increase in the use of Direct Oral Anticoagulants (DOACs), such as apixaban and rivaroxaban (see Figure 1). The Pharmacy Dosing Service was consulted on `r round(n_consult_cur_fy$pct, 0)`% of warfarin patients in FY21.
...
label_x_pos <- mdy(paste("4/15", cur_fy - 3, sep = "/"))
...
df_warf_month |>
    ...
    scale_x_date(NULL, date_breaks = "6 months", date_labels = "%b %y", limits = c(as.Date(paste(cur_fy - 3, "03-15", sep = "-")), NA)) +
...
```{r, fig.cap="Indications for oral anticoagulation. The most common indication for warfarin was for ventricular assist devices (VADs), followed by atrial fibrillation (A.fib), mechanical heart valves, deep vein thrombosis (DVTs), and pulmonary embolisms (PEs)."}
df_demog |>
    filter(fiscal_year == cur_fy) |>
...
```{r, fig.cap="The daily warfarin dose administered to patients in each group. Patients in the Pharmacy Dosing Service group received a higher daily warfarin dose than those in the traditional provider management group. The p-value is < 0.04 for each day of warfarin therapy."}
p <- df_doses_inr |>
...
```{r, fig.cap="The daily INR value for patients in each group over the first 10 days of warfarin therapy. There was a larger increase in the INR for patients managed by the Pharmacy Dosing Service. The p-value was < 0.05 on all days except days 5 and 10 where there was no significant difference."}
...
There were significantly fewer patients managed by the Pharmacy Dosing Service who were transfused with packed red blood cells (PRBC). There were no differences in supratherapeutic INR, hemoglobin drop, transfusion of fresh frozen plasma (FFP), or need for anticoagulation reversal between the two groups (see Table 1).
...
df_outcomes_pct |>
    ...
    knitr::kable(
        ...
        caption = "Outcomes and events during the first 10 days of warfarin therapy. Fewer patients managed by the Pharmacy Dosing Service were transfused with PRBC (p-value = 0.003). There were no significant differences in other outcomes."
    ) |>
    footnote(
        symbol = "p-value = 0.003",
        number = c(
            "High INR is an INR of $\\\\ge$ 5 after warfarin initiation.", 
            "Hemoglobin drop is a decrease of $\\\\ge$ 2 g/dL in a 48-hour period.", 
...
df_outcome_dispo |>
    ...
    knitr::kable(
        ...
        caption = "Discharge disposition and unplanned return within 30 days. There were significant differences in the percent of patients discharged to home, as well as the percent of patients with an unplanned visit or readmission within 30 days."
    ) |>
    pack_rows(
...
The Pharmacy Dosing Service continued to provide safe and effective management of warfarin for patients in fiscal year `r cur_fy`. 
